...
.DESCRIPTION
  This script mirrors the simple Windows `run.bat` script in a cross-platform PowerShell wrapper.
  It locates the `Ultralight-WebBrowser` executable below `build` and launches it while forwarding
  any provided command-line arguments.

.USAGE
  ./run.ps1 -- "--some-app-flag"
#>
...
Write-Host "Starting run.ps1: PowerShell $($PSVersionTable.PSVersion) Host: $($Host.Name) PID: $($PID)" -ForegroundColor Magenta
...
if (-not (Test-Path $buildPath)) {
    Write-Error "Build folder not found: $buildPath. Run the build script first (./compile.ps1)."
    exit 2
}
...
try {
  if ($Detach) {
    ...
    # Run via Invoke-Tool to capture stdout/stderr robustly
    function Invoke-Tool([string]$ExePath, [string[]]$Args) {
      Write-Host "Running (foreground): $ExePath $($Args -join ' ')" -ForegroundColor Cyan
      $outFile = [System.IO.Path]::GetTempFileName()
      $errFile = [System.IO.Path]::GetTempFileName()
      try {
        $proc = Start-Process -FilePath $ExePath -ArgumentList $Args -NoNewWindow -RedirectStandardOutput $outFile -RedirectStandardError $errFile -Wait -PassThru -ErrorAction Stop
        $exit = $proc.ExitCode
      } catch {
