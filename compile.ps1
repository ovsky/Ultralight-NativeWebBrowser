...
.DESCRIPTION
  This script replaces the Windows-only `compile.bat` with a cross-platform PowerShell script.
  It will:
  - Remove the previous cache/directory to avoid generator mismatch
  - Configure the project via `cmake -S . -B build` (appends provided arguments)
  - Build the project using `cmake --build` (Release config)
  - Optionally run `cmake --install` (Release config)

.USAGE
  ./compile.ps1 -- -DULTRALIGHT_SDK_ROOT="/path/to/sdk"
  Any arguments after `--` are passed to the `cmake` configure command.
#>
...
Write-Host "Starting compile.ps1: PowerShell $($PSVersionTable.PSVersion) Host: $($Host.Name) PID: $($PID)" -ForegroundColor Magenta
...
function Invoke-Tool([string]$Exe, [string[]]$Arguments) {
    ...
    try {
        ...
        # Fallback: run directly but avoid treating native stderr as terminating by temporarily relaxing error action
